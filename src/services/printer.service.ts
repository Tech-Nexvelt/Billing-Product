...
import { Printer, PrinterJob, PrintJobType } from '@/types/printer.types';
...
export class PrinterService extends BaseService {
  ...
  private triggerBrowserPrint(html: string): void {
  ...
  private buildKotHtml(data: KotReceiptData, paperSize: '58mm' | '80mm'): string {
  ...
  private buildBillHtml(data: BillReceiptData, paperSize: '58mm' | '80mm', template: string): string {
    const width = paperSize === '58mm' ? '52mm' : '72mm';
    ...
    return `<!DOCTYPE html><html><head><style>
      ...
      ${data.discount_amount > 0 ? `<div class="row"><span>Discount (${data.discount_rate}%)</span><span>-${sym}${data.discount_amount.toFixed(2)}</span></div>` : ''}
